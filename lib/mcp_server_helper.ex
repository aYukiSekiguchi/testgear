...
defmodule Testgear.McpServerHelper do
  @moduledoc """
  A helper module for creating MCP (Model Context Protocol) servers.

  This module provides utilities to handle MCP protocol messages including
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      defp handle_mcp_tools_list(conn, request) do
        ...
        tools = Enum.map(@mcp_tools, fn tool ->
          base = %{
            name: tool.name,
      ...
      ## Example
          ...
          def method_not_allowed(conn) do
            Testgear.McpServerHelper.method_not_allowed(conn)
          end
      """
      def method_not_allowed(conn) do
        Testgear.McpServerHelper.method_not_allowed(conn)
